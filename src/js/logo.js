...
function draw(e) {
  ...
  const totalAngle = TAU / angleDivisor;
  const waveTime = time * angleDivisor * 2;
  const radius = 200;
  const copies = 3;
  rotate(time);
  beginPath();
  arc(0, 0, radius, totalAngle, 0);
  lineWidth(4);
  stroke();
  for (let i = 0; i < samples * copies; i++) {
    const baseT = i / samples;
    const t = baseT % 1;
    const radiusWave = sin((t + floor(baseT) / copies) * TAU * 2 + waveTime);
    const _r = radius * map(radiusWave, -1, 1, 0.9, 1.111);
    const fullT = i % samples / (samples - 1);
    const r = lerp(radius, _r, sin(fullT * PI));
    const tAngle = fullT * totalAngle;
    const v = Vector.fa(tAngle, r);
    if (i % samples === 0) {
      beginPath();
    }
    lineTo(v);
